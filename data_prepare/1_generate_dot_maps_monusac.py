...
in_images_dir = os.path.join(in_root_dir, "images")
...
"""
    本脚本假设输入数据满足以下目录结构和标注格式：
        - 在 <in_root_dir> 下：
            images/{train,val,test}/：存放 patch 图像。
            annotations/points.csv：点标注，字段含 image_path, x, y, label_id, is_negative。
        - image_path 为相对 <in_root_dir> 的路径，例如 images/train/xxx.png。
...
def load_points_by_image(csv_path):
    """按 image_path 聚合点标注，跳过 is_negative=1 的样本。"""
...
if __name__ == "__main__":
    ...
    对每张图像，脚本会执行以下处理：
        1. 对 patch 图像和标注的细胞中心坐标做统一缩放。
            ...
            保存为 <image_dir>/<img_name>_img_with_dots.jpg。
        2. 生成分类点标注图，保存为 <image_dir>/<img_name>_gt_dots.npy。
        3. 生成检测点标注图，保存为 <image_dir>/<img_name>_gt_dots_all.npy。
        4. 以每个细胞中心为中心生成高斯图，并自适应设置高斯宽度，以尽量避免相邻细胞区域过度相交。
        5. 最终将高斯图转为二值掩码保存，即所有 >0 的像素置为 1，其余位置为 0。
            分类图保存为 <image_dir>/<img_name>.npy，
                每个类别还会输出对应的二值图可视化结果，文件名为 <image_dir>/<img_name>_s<class_indx>_binary.png。
            检测图保存为 <image_dir>/<img_name>_all.npy，
                对应的整体检测二值图可视化保存为 <image_dir>/<img_name>_binary.png。
    """
    ...
    if not os.path.isdir(in_images_dir):
        raise FileNotFoundError(f"未找到图像目录: {in_images_dir}")

    points_by_image = load_points_by_image(annotations_csv)

    img_files = []
    for pattern in IMAGE_GLOB_PATTERNS:
        img_files.extend(glob.glob(os.path.join(in_images_dir, pattern), recursive=True))
    ...
    for img_filepath in img_files:
        ...
        out_dir = os.path.dirname(img_filepath)
        ...
        if point_entries:
            centroids = (np.asarray([(x, y) for x, y, _ in point_entries]) * img_scale).astype(
                int
            )
        ...
        img2 = cv2.resize(
            img,
            (int(img.shape[1] * img_scale + 0.5), int(img.shape[0] * img_scale + 0.5)),
        )
            ...
            for c in range(patch_label_arr_dots.shape[-1]):
                ...
                while len(duplicate_points[0]) > 0:
                    ...
                    patch_label_arr_dots[
                        max(0, y - 2) : min(patch_label_arr_dots.shape[0] - 1, y + 3),
                        max(0, x - 2) : min(patch_label_arr_dots.shape[1] - 1, x + 3),
                        c,
                    ] = 0
